...
def room(name=None):
    ...
    return render_template('room.html', username=session['username'],
                           room=name, messages=messages)


# Message functions
@app.route('/send-message/', methods=[
    'GET'])  # Inserts the message into the database, then returns all the messages in a room
def send_message():
...
@app.route('/get-messages/',
           methods=['GET'])  # Returns all the messages in a room
def get_messages():
    room = request.args.get('room', '')
